...
**Integration tests** for the [`GroupDocs.Signature.Mcp`](https://www.nuget.org/packages/GroupDocs.Signature.Mcp) NuGet package — an MCP server that exposes GroupDocs.Signature for .NET as AI-callable tools.

This repo is **not** the server itself. The server lives at [groupdocs-signature/GroupDocs.Signature.Mcp](https://github.com/groupdocs-signature/GroupDocs.Signature.Mcp). This repo:
...
```
src/GroupDocs.Signature.Mcp.Tests/
  Fixtures/
  ...
  GroupDocs.Signature.Mcp.Tests.csproj
...
# Run all ~27 tests against the default package version (26.7.0)
...
- Do not add a `ProjectReference` to the main repo's `GroupDocs.Signature.Mcp.csproj`. This repo exists to test the shipped NuGet, not the source.
