# GroupDocs.Signature.Mcp.Tests

Integration tests for the [`GroupDocs.Signature.Mcp`](https://www.nuget.org/packages/GroupDocs.Signature.Mcp)
NuGet package — an MCP server that exposes
[GroupDocs.Signature](https://products.groupdocs.com/signature) as AI-callable tools.
...
| Area | Covered by |
|---|---|
| Package installs and starts via `dnx` | [McpServerFixture](src/GroupDocs.Signature.Mcp.Tests/Fixtures/McpServerFixture.cs) |
| MCP handshake, server info, 8-tool advertisement | [ToolDiscoveryTests](src/GroupDocs.Signature.Mcp.Tests/ToolDiscoveryTests.cs) |
| `Sign` — text / qrcode / barcode + signed-output presence | [SignTests](src/GroupDocs.Signature.Mcp.Tests/SignTests.cs) |
| `Verify` — JSON shape + per-type parameter | [VerifyTests](src/GroupDocs.Signature.Mcp.Tests/VerifyTests.cs) |
| `SearchTextSignatures` + filter | [SearchTextSignaturesTests](src/GroupDocs.Signature.Mcp.Tests/SearchTextSignaturesTests.cs) |
| `SearchBarcodes` + filter | [SearchBarcodesTests](src/GroupDocs.Signature.Mcp.Tests/SearchBarcodesTests.cs) |
| `SearchQrCodes` + filter | [SearchQrCodesTests](src/GroupDocs.Signature.Mcp.Tests/SearchQrCodesTests.cs) |
| `SearchDigitalSignatures` | [SearchDigitalSignaturesTests](src/GroupDocs.Signature.Mcp.Tests/SearchDigitalSignaturesTests.cs) |
| `SearchImageSignatures` | [SearchImageSignaturesTests](src/GroupDocs.Signature.Mcp.Tests/SearchImageSignaturesTests.cs) |
| `GetDocumentInfo` — JSON shape + per-format theory | [GetDocumentInfoTests](src/GroupDocs.Signature.Mcp.Tests/GetDocumentInfoTests.cs) |
| Unknown / corrupted files, password parameter | [ErrorHandlingTests](src/GroupDocs.Signature.Mcp.Tests/ErrorHandlingTests.cs) |
...
- name: Dispatch smoke tests
  ...
  run: |
    gh api repos/groupdocs-signature/GroupDocs.Signature.Mcp.Tests/dispatches \
...
pages and prefixes its response with `"[Evaluation mode]"`. All 8 tools
...
- **Synthetic** — a blank PDF and a 1×1 JPEG built from byte-arrays in
  [SampleDocuments.cs](src/GroupDocs.Signature.Mcp.Tests/Fixtures/SampleDocuments.cs)
  at test startup. No binaries committed.
- **Real samples** — minimal PDF / DOCX / XLSX / JPG / PNG copied from the
  upstream GroupDocs.Signature examples repo and committed under
