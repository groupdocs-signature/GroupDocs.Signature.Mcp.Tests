...
Complete bash script suite for running GroupDocs.Signature.Mcp integration tests in both local and Docker environments. **Works on Windows (Git Bash/WSL), Linux, and macOS.**

## 📋 Overview

This folder contains production-ready bash scripts for testing the GroupDocs.Signature.Mcp NuGet package across all scenarios:

- **ToolDiscovery** — MCP server handshake and the 8-tool listing
- **Sign** — apply text / QR code / barcode / digital-certificate signatures, write `<name>_signed.<ext>`
- **Verify** — return JSON validity report for text / QR / barcode / digital signatures
- **SearchTextSignatures / SearchBarcodes / SearchQrCodes / SearchDigitalSignatures / SearchImageSignatures** — locate embedded signatures with optional text filters
...
| `04_run-server-with-samples.sh` | **Interactive Docker MCP server** — Launches the published `signature-net-mcp` container with `Files/` mounted, for ad-hoc smoke testing. |
...
**Output:**
```
Passed  ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsSignatureMcp
...
services:
  signature-server:
    image: ghcr.io/groupdocs-signature/signature-net-mcp:latest
    volumes:
...
Without a license:
- `Verify_InEvaluationMode_ReturnsErrorResponse` — ✅ Passes (asserts graceful error)
- `Verify_Jpeg_WritesCleanOutput_Licensed` — ⏭️ Skipped
- `Verify_FollowUpReadOfCleanedFile_Succeeds_Licensed` — ⏭️ Skipped
...
docker build -t signature-net-mcp:local ../../../GroupDocs.Signature.Mcp/
...
# Docker utilities
docker_pull_image "ghcr.io/groupdocs-signature/signature-net-mcp:latest"
check_docker_daemon
...
- [GroupDocs.Signature.Mcp NuGet](https://www.nuget.org/packages/GroupDocs.Signature.Mcp)
- [Integration Tests Guide](../how-to/06-run-integration-tests.md)
- [Docker Setup Guide](../how-to/02-run-via-docker.md)
- [Test Project](../src/GroupDocs.Signature.Mcp.Tests/GroupDocs.Signature.Mcp.Tests.csproj)
