...
Two install routes from [`nuget.org/packages/GroupDocs.Signature.Mcp`](https://www.nuget.org/packages/GroupDocs.Signature.Mcp):
...
```bash
dnx GroupDocs.Signature.Mcp --yes                # latest stable, refreshed every run
dnx GroupDocs.Signature.Mcp --prerelease --yes   # latest including pre-releases
```
...
The cache lives at `%USERPROFILE%\.nuget\packages\groupdocs.signature.mcp\<version>\`
on Windows and `~/.nuget/packages/groupdocs.signature.mcp/<version>/` on
...
info: ModelContextProtocol.Server.StdioServerTransport[...]
      Server (stream) (GroupDocs.Signature.Mcp) transport reading messages.
...
(
  ...
  sleep 2
) | GROUPDOCS_MCP_STORAGE_PATH=./docs dnx GroupDocs.Signature.Mcp@26.5.0 --yes
```

You should see two JSON-RPC responses containing `sign` and
`verify`.
...
```bash
dotnet tool update -g GroupDocs.Signature.Mcp
```

To uninstall:

```bash
dotnet tool uninstall -g GroupDocs.Signature.Mcp
```

The tool runs from `~/.dotnet/tools/groupdocs-signature-mcp` (Linux/macOS) or
`%USERPROFILE%\.dotnet\tools\groupdocs-signature-mcp.exe` (Windows). Make sure
...
| `GROUPDOCS_LICENSE_PATH` | Path to `GroupDocs.Total.lic` — required for `verify` | *(evaluation mode)* |
...
All eight tools work in evaluation mode. `verify`, the `search*` tools, and
`getDocumentInfo` return read-only results unchanged regardless of license.
`sign` produces signed output in evaluation mode too, but with a
diagnostic watermark on the signed pages and a `"[Evaluation mode] Output may
include watermarks."` prefix in the tool response.

Get a license from [purchase.groupdocs.com](https://purchase.groupdocs.com/).
Point `GROUPDOCS_LICENSE_PATH` at the `.lic` file and `sign` output
loses both the watermark and the evaluation-mode prefix.
...
```text
initialize response → serverInfo: { name: "GroupDocs.Signature.Mcp", version: "26.5.0" }
```
...
| `No license configured. Running in evaluation mode.` | No `GROUPDOCS_LICENSE_PATH` | Expected. Set the path to drop the `[Evaluation mode]` prefix and the watermark on `sign` output. |
| `[Evaluation mode] Output may include watermarks.` | `sign` without license | Expected in eval mode. Set `GROUPDOCS_LICENSE_PATH` to remove watermarks. |
