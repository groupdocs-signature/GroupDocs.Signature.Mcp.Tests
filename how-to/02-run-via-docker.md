...
- `ghcr.io/groupdocs-signature/signature-net-mcp` — GitHub Container Registry (primary)
- `docker.io/groupdocs/signature-net-mcp` — Docker Hub (mirror)
...
docker run --rm -i \
  ...
  ghcr.io/groupdocs-signature/signature-net-mcp:26.5.0
```
...
```bash
docker pull ghcr.io/groupdocs-signature/signature-net-mcp:26.5.0   # pinned to exact version
docker pull ghcr.io/groupdocs-signature/signature-net-mcp:latest   # floats to most recent push
```
...
docker pull ghcr.io/groupdocs-signature/signature-net-mcp:latest
...
```bash
docker inspect ghcr.io/groupdocs-signature/signature-net-mcp:latest \
...
(
  ...
  sleep 2
    ...
    -v $(pwd)/documents:/data \
    ghcr.io/groupdocs-signature/signature-net-mcp:26.5.0 2>/dev/null
```

Expected: two JSON-RPC responses on stdout. The second includes `sign`
and `verify` with their descriptions and input schemas.
...
services:
  groupdocs-signature-mcp:
    image: ghcr.io/groupdocs-signature/signature-net-mcp:26.5.0
    stdin_open: true
    tty: false
    environment:
...
Without the license, `verify` returns the evaluation-mode error
...
docker inspect ghcr.io/groupdocs-signature/signature-net-mcp:26.5.0 \
  --format '{{json .Config}}' | jq

# Expected:
# - Entrypoint: ["dotnet", "GroupDocs.Signature.Mcp.dll"]
...
| `manifest unknown` / can't pull image | Version tag doesn't exist on that registry | Check [ghcr.io/groupdocs-signature/signature-net-mcp](https://github.com/groupdocs-signature/GroupDocs.Signature.Mcp/pkgs/container/signature-net-mcp) for available tags. |
| `:latest` ran but didn't pick up a new release | Docker reuses cached image — `docker run :latest` does not auto-pull | `docker pull ghcr.io/groupdocs-signature/signature-net-mcp:latest` before running, or add `--pull always` to your `docker run`. |
