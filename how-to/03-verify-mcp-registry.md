...
The `GroupDocs.Signature.Mcp` package is an **MCP server** (the csproj declares
...
[nuget.org/packages/GroupDocs.Signature.Mcp](https://www.nuget.org/packages/GroupDocs.Signature.Mcp)
...
- `command: "dnx"`
- `args: ["GroupDocs.Signature.Mcp@26.5.0", "--yes"]`
...
```bash
curl -L -o pkg.nupkg \
  https://www.nuget.org/api/v2/package/GroupDocs.Signature.Mcp/26.5.0
...
```json
{
  ...
  "name": "io.github.groupdocs-signature/groupdocs-signature-mcp",
  "version": "26.5.0",
  "packages": [ { "registryType": "nuget", "identifier": "GroupDocs.Signature.Mcp", ... } ],
  "repository": { "url": "https://github.com/groupdocs-signature/GroupDocs.Signature.Mcp" }
}
```
...
3. Search for `groupdocs-signature` or `signature`.
...
- `dnx` successfully resolves `GroupDocs.Signature.Mcp@26.5.0` from nuget.org.
...
curl -s https://registry.modelcontextprotocol.io/v0/servers \
  | jq '.[] | select(.name | contains("groupdocs-signature"))'
```
...
- `ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsSignatureMcp` — the server
  reports `serverInfo.name == "GroupDocs.Signature.Mcp"` and a non-empty version
