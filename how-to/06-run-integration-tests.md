...
This repo's test suite validates the **published** `GroupDocs.Signature.Mcp`
...
and prefixes its response with `"[Evaluation mode]"`. All 8 tools still
...
Synthetic PDF/JPEG fixtures ([SampleDocuments.cs](../src/GroupDocs.Signature.Mcp.Tests/Fixtures/SampleDocuments.cs))
...
- name: Dispatch integration tests
  ...
  run: |
    gh api \
      repos/groupdocs-signature/GroupDocs.Signature.Mcp.Tests/dispatches \
...
base64 -w0 GroupDocs.Total.lic | gh secret set GROUPDOCS_LICENSE \
  --repo groupdocs-signature/GroupDocs.Signature.Mcp.Tests
```
...
(
  ...
  sleep 5
) | GROUPDOCS_MCP_STORAGE_PATH=/tmp/gd dnx GroupDocs.Signature.Mcp@26.7.0 --yes \
